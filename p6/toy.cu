...
void getDeviceProperties(const int devID, cudaDeviceProp *deviceProp)
{
  ...
  fprintf(stderr, "Leyendo propiedades del dispositivo %d...\n", devID);
  cudaErrorHandler(cudaGetDeviceProperties(deviceProp, devID), __LINE__);

  fprintf(stderr, "GPU Device %d: \"%s\": capacidad de cómputo %d.%d.\n\n",
                     ...
                     int **colptr, int **rowind, double **values)
{
  ...
  fprintf(stderr, "Reading input matrix from %s...\n", matrixFile);
  readHB_newmat_double(matrixFile, nrow, ncol, nnzero,
                       colptr, rowind, values);

  fprintf(stderr, "Matrix in file %s is %d x %d ", matrixFile, *nrow, *ncol);
  fprintf(stderr, "with %d nonzero elements.\n\n", *nnzero);
}
                       ...
                       int *colptr, int *rowind, double *values)
{
  ...
  fprintf(stderr, "Writing output matrix in %s...\n", matrixFile);
  ...
  fprintf(stderr, "Generated file %s successfully.\n\n", matrixFile);
}
...
__global__ void kernelAdd(TYPE *dvalues, int numOperations,
...
int main(int argc, char **argv)
{
  ...
  int *colptr, *rowind;
  TYPE *values;  // POINT 4: Puedes usar int para medir el rendimeinto en operaciones en punto fijo
  ...
  int *blocks;
  int *threads;
  TYPE *dvalues;  // POINT 5: This declaration is binded to hints MU2 and MU4
  ...
  // Ejecución
  fprintf(stderr, "Running mytoy.\n");

  //-------------------------------------------------------------------------
  // Copy matrix values from host memory to device memory.

  // POINT 9: Hay que adecuar el tipo de dato a float, int o double (Ver puntos 2, 4, 5, 6 y 7)
  int valuesSize = nnzero * sizeof(TYPE);
  ...
  // 	POINT 10: Si crees que un sólo stream es mejor para toda la matriz,
  ...
  for (int i=0; i<ncol; ++i) { // POINT 11: La forma en la que se despliega el paralelismo está aquí.
  ...
  fprintf(stderr, "Streams executed successfully.\n");

  //-------------------------------------------------------------------------
  // Copiar los resultados de vuelta a la CPU.
  cudaErrorHandler(cudaEventRecord(start, NULL), __LINE__);

  fprintf(stderr, "Copiando los valores de vuelta desde la... ");
  fprintf(stderr, "memoria del dispositivo hasta la memoria del host...\n\n");
  ...
  fprintf(stderr, "Tiempo de acceso a la memoria de la GPU: %.4f ms.\n\n", msecMem);

  fprintf(stderr, "Creación de streams en la GPU:  %.4f ms.\n", msecCompStr);
  fprintf(stderr, "Tiempo de ejecución del kernel: %.4f ms.\n", msecCompKrn);
  fprintf(stderr, "Tiempo de computación en GPU:   %.4f ms.\n\n", msecComp);

  //POINT 12:	Cambia float, int or double según el punto 2, 4, 5, 6, 7 y 8
  opIntensity = numOperationsPerValue / sizeof(TYPE);
  fprintf(stderr, "Operaciones en punto flotante por byte: %.4f FLOP/byte.\n", opIntensity);
  ...
  fprintf(stderr, "Rendimiento: %.4f GFLOP/s.\n\n", gigaFlops);

  fprintf(stdout, "%.4f, %.4f, %.4f, %.4f, %.4f, %.4f",
          msecMem, msecCompStr, msecCompKrn, msecComp, opIntensity, gigaFlops);
